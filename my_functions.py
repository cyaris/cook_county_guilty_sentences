...
def cm_val_scaled(model, threshold = 0.5):
   # Predict class 1 if probability of being in class 1 is greater than threshold
   # (model.predict(X_test) does this automatically with a threshold of 0.5)
    y_predict = (model.predict_proba(X_validation_2_scaled)[:, 1] >= threshold)
    confusion = metrics.confusion_matrix(y_validation_2, y_predict)
    plt.figure(dpi = 80)
...
def cm_val(model, threshold = 0.5):
   # Predict class 1 if probability of being in class 1 is greater than threshold
   # (model.predict(X_test) does this automatically with a threshold of 0.5)
    y_predict = (model.predict_proba(X_validation_2)[:, 1] >= threshold)
    confusion = metrics.confusion_matrix(y_validation_2, y_predict)
    plt.figure(dpi = 80)
    sns.heatmap(confusion, cmap = plt.cm.Blues, annot = True, square = True, fmt = 'd',
                xticklabels=['Plead Guilty', 'Found Guilty'],
